					...
					data.variants.forEach(function (variant) {
						...
						if (values.indexOf(value) == -1) {						
							...
							var swatchItem = '<a href="{{itemUrl}}?variant={{variantId}}" ' +
													'class="color-swatch color-swatch--with-image color-swatch--small {{swatchClass}}" '+
													'data-variant-id="{{variantId}}" ' +
													'data-variant-image="{{variantImage}}" ' +
													'>'+
                                				'<span class="color-swatch--icon color-swatch--{{colorValueSlugify}}" style="background-image: url({{colorImage}}); background-color: {{backgroundColor}};"></span>'
												'</a>';
